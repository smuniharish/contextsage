...
* If the LLM summarization step itself failed (timeout, rate limit,
  malformed response after retries), ContextIQ falls back to a purely
...
* If validation failed (a required fact went missing from an otherwise
  successful summary), ContextIQ appends a literal "preserved facts"
...
from contextiq.core.models import RecoveryOutcome, ValidationResult
...
_PRESERVED_FACTS_HEADER = (
    "The following facts were flagged as must-preserve by ContextIQ and are "
...
class RecoveryManager:
    """Implements ContextIQ's staged, fail-safe recovery strategies."""

    def build_preserved_facts_message(self, facts: tuple[str, ...]) -> AIMessage:
        bullet_list = "\n".join(f"- {fact}" for fact in facts)
        return AIMessage(
            id=f"contextiq-recovery-{uuid.uuid4().hex[:8]}",
