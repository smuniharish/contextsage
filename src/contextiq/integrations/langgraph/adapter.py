...
ContextIQ never reimplements LLM-based semantic summarization.
...
*prepared* message list that ContextIQ has already decomposed, scored, and
selectively transformed.

Design note: the wrapped middleware is configured with a trigger that is
effectively "always eligible" (``("tokens", 0)``); ContextIQ's own
:class:`~contextiq.budget.analyzer.BudgetAnalyzer` and
:class:`~contextiq.planning.planner.SummarizationPlanner` are the sole
...
from contextiq.exceptions import SummarizationError
...
ContextIQ's own budget analyzer and planner are the authority on *whether*
