...
This is the single public entry point of ContextIQ.
...
from contextiq.budget.tokens import TokenCounter, create_default_token_counter
from contextiq.classification.signals import StructuralSignalDetector
from contextiq.context.decomposition import ContextDecomposer
from contextiq.context.observer import ContextObserver
from contextiq.core.models import RecoveryOutcome, RelationType
...
from contextiq.integrations.langgraph.adapter import LangGraphSummarizationAdapter
from contextiq.integrations.langgraph_xai.provenance import ProvenanceManager
from contextiq.lineage.manager import LineageManager, find_prior_summary_ids
...
from contextiq.planning.trigger import TriggerSpec, evaluate_trigger
from contextiq.preservation.engine import Policy as PreservationPolicy
...
class IntelligentSummarizationMiddleware(AgentMiddleware):
    ...
    Args:
        ...
        trigger: When summarization should fire, e.g. ``("tokens", 100_000)``
            or ``("fraction", 0.8)``. See :mod:`contextiq.planning.trigger`.
        ...
        policy: Preservation aggressiveness — ``"maximum_preservation"``,
            ...
            :mod:`contextiq.preservation.engine`.
        ...
        observability_hook: Custom :class:`~contextiq.observability.events.ObservabilityHook`
            to receive :class:`~contextiq.observability.events.SummarizationEvent`
        ...
        token_counter: Custom :class:`~contextiq.budget.tokens.TokenCounter`
            (defaults to a ``cl100k_base`` ``TiktokenTokenCounter``).
        parsers: Custom :class:`~contextiq.parsers.base.ContentParser`
        ...
        severity_pattern: Custom regex overriding how log/error severity
            ...
            :data:`~contextiq.classification.signals.DEFAULT_SEVERITY_PATTERN`.
        error_keywords_pattern: Custom regex overriding which keywords mark
            ...
            :data:`~contextiq.classification.signals.DEFAULT_ERROR_KEYWORDS_PATTERN`.
        identifier_patterns: Custom regexes overriding which substrings are
            ...
            :data:`~contextiq.classification.signals.DEFAULT_IDENTIFIER_PATTERNS`.
