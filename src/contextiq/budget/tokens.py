...
ContextIQ needs token counts in two places with different cost profiles:

* **Budget analysis** (whole-conversation accounting) — should match, as
  closely as possible, whatever the wrapped LangGraph
  ``SummarizationMiddleware`` itself uses, so ContextIQ's "is summarization
...
from contextiq.exceptions import TokenCountingError
...
#: explicit timeout (see ``tiktoken.load.read_file``). To keep ContextIQ
...
class TokenCounter(ABC):
    """Abstract base class for every token counting strategy in ContextIQ.
    ...
    ContextIQ code needs to change.
    """
...
class TiktokenTokenCounter(TokenCounter):
    ...
    thing ContextIQ has to a provider-verified count without requiring a
