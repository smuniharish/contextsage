...
summarized already include a prior ContextSage-produced summary.
"""
...
from contextsage.core.models import SummaryLineage

CONTEXTSAGE_SUMMARY_ID_PREFIX = "contextsage-summary-"
...
class LineageManager:
    ...
    def next_summary_id(self) -> str:
        return f"{CONTEXTSAGE_SUMMARY_ID_PREFIX}{uuid.uuid4().hex[:12]}"
...
def find_prior_summary_ids(message_ids: tuple[str | None, ...]) -> tuple[str, ...]:
    """Identify which of the given message ids are themselves prior ContextSage summaries."""
    return tuple(
        mid for mid in message_ids if mid and mid.startswith(CONTEXTSAGE_SUMMARY_ID_PREFIX)
    )
