"""Core internal domain models shared across ContextSage's engines.
...
exported from :mod:`contextsage` — the public surface of the package is
:class:`contextsage.IntelligentSummarizationMiddleware` only (see
...
class RegionKind(StrEnum):
    ...
    ContextSage never assumes a whole message is one type. A
    ...
    # ``RegionKind`` covers the built-in structural kinds ContextSage ships
    # with detectors for. Because it is a ``StrEnum``, any plain string is
    # also a valid runtime value for ``ContentSignals.kind`` (see its
    # ``RegionKind | str`` annotation) — a custom :class:`~contextsage.parsers.base.ContentParser`
...
class SummarizationPlan:
    """The output of the :class:`~contextsage.planning.planner.SummarizationPlanner`."""
...
class RecoveryOutcome:
    """Result of a :class:`~contextsage.recovery.manager.RecoveryManager` attempt."""
